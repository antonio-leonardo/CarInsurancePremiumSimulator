...
def _patch_get(monkeypatch: pytest.MonkeyPatch, handler) -> None:
...
def test_valid_response(monkeypatch: pytest.MonkeyPatch) -> None:
    def handler(url, **kwargs):
        ...
        return httpx.Response(200, json={"adjustment": 0.01}, request=httpx.Request("GET", url))
...
def test_out_of_range_fail_closed(monkeypatch: pytest.MonkeyPatch) -> None:
    def handler(url, **kwargs):
        return httpx.Response(200, json={"adjustment": 0.5}, request=httpx.Request("GET", url))
...
def test_missing_key_fail_open(monkeypatch: pytest.MonkeyPatch) -> None:
    def handler(url, **kwargs):
        return httpx.Response(200, json={"nope": 1}, request=httpx.Request("GET", url))
